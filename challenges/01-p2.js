...
function extractNumbersFromLine(line) {
    ...
    const letterHandler = (letter) => {
        for (const word in memoryWords) {
            const wordLength = word.length;
            let wordCounter = memoryWords[word];
            let currentChar = word[wordCounter];

            // reset the word counter if the current character is not the same as the current word character
            if (currentChar !== letter) {
                wordCounter = 0;
                memoryWords[word] = wordCounter;
                currentChar = word[wordCounter];
            }

            // verify if the current character is the same as the current word character
            if (currentChar === letter) {
                wordCounter++;
                memoryWords[word] = wordCounter;

                if (wordCounter === wordLength) {
                    numbersInLine.push(wordToNumber[word]);
                    memoryWords[word] = 0;
                }
            }
        }
    ...
    // Counting the number words in the line
    line.split('').forEach(letter => {
        if (!isNaN(letter))
            numbersInLine.push(parseInt(letter))
        else
...
function solution(input) {
    ...
    lines.forEach(line => {
        ...
        const numbersInLine = extractNumbersFromLine(line);

        // Finding the first and last digits among all extracted numbers
        if (numbersInLine.length > 0) {
            const firstDigit = numbersInLine[0];
            const lastDigit = numbersInLine[numbersInLine.length - 1];

            // Verify if both digits were found
...
const calibrationDocument = `9dlvndqbddghpxc
...
const totalSum = solution(calibrationDocument);
